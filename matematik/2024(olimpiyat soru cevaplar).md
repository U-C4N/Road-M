...
# Problem
Rasyonel sayıların kümesi \( Q \) üzerinde fonksiyonları inceliyoruz. Bir fonksiyon \( f: Q \to Q \), her \( x, y \in Q \) için aşağıdaki şartlardan en az birini sağlıyorsa, bu fonksiyona aquaesulian fonksiyonu denir:
1. `f(x + f(y)) = f(x) + y`
2. `f(f(x) + y) = x + f(y)`
...
**İddia 2:** Her \( x \in Q \) için, `f(-f(x)) = f(0) - x`.
...
**İddia 3:** Her \( r \in Q \) için, `f(r) + f(-r)` ifadesi en fazla iki farklı değer alabilir.
...
### Örnek:
Aşağıdaki fonksiyonun aquaesulian olduğunu gösterelim: `f(x) = floor(2x) - x`, burada `floor()` tam kısmı alır.
Bu fonksiyon için:
`f(r) + f(-r) = (floor(2r) - r) + (floor(-2r) + r) = floor(2r) + floor(-2r)`
Eğer \( 2r \) bir tam sayıysa, `floor(2r) + floor(-2r) = -1`.
Aksi halde, `floor(2r) + floor(-2r) = -2`.
